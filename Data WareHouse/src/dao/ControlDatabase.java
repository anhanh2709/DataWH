...
public class ControlDatabase {
	...
	public String selectField(String field, String conditon) {
		...
		try {
			pst = DBConnection.getConnectionFromControl(this.config_db_name).prepareStatement(sql);
	...
	public boolean tableExist(String table_name) throws ClassNotFoundException {
		try {
			DatabaseMetaData dbm = DBConnection.getConnectionFromControl(this.target_db_name).getMetaData();
	...
	public boolean insertValues(String column_list, String values, String target_table) throws ClassNotFoundException {
		...
		try {
			pst = DBConnection.getConnectionFromControl(this.target_db_name).prepareStatement(sql);
			pst.executeUpdate();
			return true;
		} catch (SQLException e) {
			e.printStackTrace();
